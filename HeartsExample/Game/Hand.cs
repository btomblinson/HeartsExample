            ...
            while (!CurrentGame.Players.Any(x => x.GamePoints >= 50))
            {
                ...
                while (CurrentTrick.TrickNumber < 14)
                {
                    for (int playerCount = 1; playerCount <= 4; playerCount++)
                    {
                        ...
                        Card playerCard = player.DetermineCardToPlay(CurrentTrick, cardsInTrick, startingCard);

                        if (!CurrentTrick.CardIsValidForTrick(cardsInTrick, playerCard, player.PlayerCards, startingCard))
                        {
                    ...
                    //do some checks
                    if (cardsInTrick.Count != 4)
                    {
                        throw new Exception("Not enough cards played in trick! ");
                    }

                    //determine who won trick and reset order
                    Tuple<BasePlayer, Card> trickWinner = CurrentTrick.DetermineTrickWinner(cardsInTrick, startingCard);
                    CurrentTrick.DetermineTrickPoints(trickWinner.Item1, cardsInTrick);
